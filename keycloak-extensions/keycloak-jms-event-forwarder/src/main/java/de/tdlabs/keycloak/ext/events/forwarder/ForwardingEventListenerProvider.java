...
public class ForwardingEventListenerProvider implements EventListenerProvider {
	...
	public void close() {
		...
		LOG.tracev("Closing: component={0}", Utils.toComponentIdString(this));

		if (idmEventMapper != null) {
			LOG.tracev("Closing: component={0}", Utils.toComponentIdString(idmEventMapper));
			idmEventMapper.close();
			LOG.tracev("Closed: component={0}", Utils.toComponentIdString(idmEventMapper));
		}
	}
